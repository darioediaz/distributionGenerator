...
public class RandomDistributionGUI extends JFrame {
  ...
  private final JTable frequencyTable;

  public RandomDistributionGUI() {
    setLookAndFeel();
    ...
    inputGbc.gridx = 1;
    distributionBox = new JComboBox<>(new String[]{"Uniforme", "Exponencial", "Normal Box-Muller", "Normal Convolución"});
    ...
    JButton generateButton = new JButton("Generar Distribución");
  ...
  private void generateDistribution() {
    try {
      ...
      showFrequencyTable(samples, bins);
  ...
  private boolean validateParameters(int option) {
    try {
      ...
      if ((option == 0 || option == 1 || option == 2) && (sampleSize == 1)) {
        showError("EL tamaño de muestra para esta distribución debe ser al menos 2.");
        return false;
      }
      if (option == 3 && sampleSize < 12) {
        showError("EL tamaño de muestra para esta distribución debe ser al menos 12.");
        return false;
      }
    } catch (NumberFormatException e) {
  ...
  private void showFrequencyTable(double[] data, int bins) {
    int option = distributionBox.getSelectedIndex();
    double min, max;
  ...
  public static void main(String[] args) {
    SwingUtilities.invokeLater(() -> {
      RandomDistributionGUI gui = new RandomDistributionGUI();
